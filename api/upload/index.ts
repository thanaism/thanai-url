...
import { BlobServiceClient, BlockBlobUploadOptions, BlockBlobUploadResponse } from '@azure/storage-blob'
import HTTP_CODES from 'http-status-enum';
import { pickAlias, validateAlias } from './alias';
import { ALIAS_DEFAULT_LENGTH } from './constant';
import { AzureFunctionsEnvironment, validateConnectionString, isUrlString, LinkType, getLinkType } from './utilities';
...
const httpTrigger: AzureFunction = async function (context: Context, req: HttpRequest): Promise<void> {
  ...
  if (res == null) {
    ...
    // No response code can be set without `res`, so there is no way to notify the caller other than an error.
  ...
  if (BSW_BASE_URL == null || !isUrlString(BSW_BASE_URL) ||
    CDN_BASE_URL == null || !isUrlString(CDN_BASE_URL)) {
    res.status = HTTP_CODES.INTERNAL_SERVER_ERROR;
    context.log.error(`Environment variable "BlobStaticWebSiteBaseUrl" or "CdnBaseUrl" isn't configured correctly.`);
    return;
  }
  ...
  if (longUrl == null || !isUrlString(longUrl)) {
    res.body = { message: `Query parameter "url" isn't configured correctly.` };
  ...
  if (userDefinedAlias != null && !(await validateAlias(userDefinedAlias))) {
    res.body = { message: `This "alias" is already in use or in the wrong format.` };
    res.status = HTTP_CODES.BAD_REQUEST;
    context.log.warn(res.body.message);
    return;
  }

  const blobName: string = userDefinedAlias != null ? userDefinedAlias : await pickAlias(ALIAS_DEFAULT_LENGTH);
  const blockBlobClient = containerClient.getBlockBlobClient(blobName);
  context.log("\nUploading to Azure storage as blob:\n\t", blobName)

  try {
    const data = `<!DOCTYPE html><head><title>redirect</title>`
